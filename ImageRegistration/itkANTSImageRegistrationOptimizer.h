...
//     m_Debug = false;
    for( int idim = 0; idim < ImageDimension; idim++ )
      ...
      if( this->m_Debug )
        {
        ::ants::antscout << " ExpFac " << expandFactors[idim] << " curdsz " << this->m_CurrentDomainSize[idim] << std::endl;
        }
      }
...
// set up parameters for deformation restriction
    ...
    if( thicknessOption->GetNumberOfFunctions() && ( thicknessOption->GetFunction( 0 )->GetName() == "true" ||  thicknessOption->GetFunction( 0 )->GetName() == "1" ) )
      {
...
//      itkExceptionMacro( "Invalid regularization: " << regularizationOption->GetFunction( 0 )->GetName() );
  ...
  void ComputeMultiResolutionParameters(ImagePointer fixedImage )
  {
    ...
    if( this->m_UseROI )
      ...
      ::ants::antscout << " ROI #s : size " << this->m_FullDomainSize << " orig " <<   this->m_FullDomainOrigin  << std::endl;
      }
...
//      ::ants::antscout <<" totale " << totale << std::endl;
        ...
        for( unsigned int qq = 0; qq < this->m_Energy.size(); qq++ )
          ...
          ::ants::antscout << " energy " << qq << " : " << this->m_Energy[qq]; //  << " Last " << this->m_LastEnergy[qq];
...
//                    index[ii]=origindex[ii]-cornerind[ii];
      ...
      if( oktosample )
        {
        //      ::ants::antscout << " index " << index <<  " origindex " << origindex << " ok? " << oktosample << std::endl;
