...
/** Get the mask if there is one */
  ...
  if( fixed_initial_affine_filename != "" )
    ...
    << std::endl;
    std::string refheader = this->m_Parser->GetOption( "fixed-image-initial-affine-ref-image" )->GetFunction( 0 )->GetName();
    if( refheader != "" )
      {
  ...
  bool useNN = this->m_Parser->template Convert<bool>( this->m_Parser->GetOption( "use-NN" )->GetFunction()->GetName() );
  if( useNN )
    {
