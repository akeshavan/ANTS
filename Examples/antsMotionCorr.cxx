...
int ants_motion( itk::ants::CommandLineParser *parser )
{
  ...
  for( int currentStage = numberOfStages - 1; currentStage >= 0; currentStage-- )
    ...
    std::vector<float> sigmas = parser->ConvertVector<float>( smoothingSigmasOption->GetFunction( currentStage )->GetName()  );
    ...
    for( unsigned int timelistindex = 0;  timelistindex < timelist.size();  timelistindex++ )
      ...
      if( std::strcmp( whichMetric.c_str(), "cc" ) == 0 )
        ...
        unsigned int npoints_to_skip = static_cast<unsigned int>( 1 / std::atof( ( metricOption->GetFunction( currentStage )->GetParameter(  5 ) ).c_str() ) );
	...
	metric->Initialize();
        ...
        // Extract parameters
        RealType sigmaForUpdateField = parser->Convert<float>( transformOption->GetFunction( currentStage )->GetParameter(  1 ) );
        RealType sigmaForTotalField = parser->Convert<float>( transformOption->GetFunction( currentStage )->GetParameter(  2 ) );
