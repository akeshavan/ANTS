...
int BlobDetector( int argc, char *argv[] )
{
  ...
  if ( !blobs2.empty() && ! ( blobs1.empty() ) )
	...
	count2++;
      ...
      if (  ( maxcorr > 0.75 ) && ( bestblob )  && ( image->GetPixel( indexi )   > 1.e-4  )  && ( image2->GetPixel( bestblob->GetCenter() )  > 1.e-4  ) )
	{
	antscout << " best correlation " << maxcorr << " rad1 " << ( *i )->GetObjectRadius() << " rad2 " << bestblob->GetObjectRadius( ) << " : " << (RealType) count1 / (RealType) nblobs * 100.0 << "% " << matchpt << " voxval1 " << image->GetPixel( indexi ) << " voxval2 " << image2->GetPixel( bestblob->GetCenter() ) << std::endl;
