...
{	
  ...
  if ( ImageDimension == 2 )
    ...
    for( unsigned int ii = 0; ii < Gsz; ii++ )
      ...
      for( unsigned int dd = 0; dd < ImageDimension; dd++ ) ptran[ dd ] -= avgpoint2[ dd ];
      // rotate ptran using 2D equation 
      PointType ptran2;
      ptran2[ 0 ] = ptran[ 0 ] * cos( theta ) - ptran[ 1 ] * sin( theta );
      ptran2[ 1 ] = ptran[ 1 ] * cos( theta ) + ptran[ 0 ] * sin( theta );
      for( unsigned int dd = 0; dd < ImageDimension; dd++ ) ptran[ dd ] = ptran2[ dd ] + avgpoint2[ dd ];
...
int BlobDetector( int argc, char *argv[] )
{
  ...
  if ( !blobs1.empty() )
    for( typename BlobsListType::const_iterator i = blobs1.begin(); i != blobs1.end(); ++i)
      {
      //	antscout <<  "Size: " << (*i)->GetObjectRadius() << " sigma of Laplacian detector " << (*i)->GetScaleSpaceSigma() << " sigma of detected Gaussian " << (*i)->GetSigma() << " center "  <<  (*i)->GetCenter() << std::endl;
      }
  antscout << std::endl;
  if ( fn2.length() > 3 ) 
    {
  ...
  if ( !blobs2.empty() && ! ( blobs1.empty() ) )
	...
	if ( csum > 0 ) correspondencematrix.set_column( ii, correspondencematrixcol / csum );
	}
	...
	if ( rsum > 0 ) correspondencematrix.set_row( ii, correspondencematrixrow / rsum );
	}
      }
    ...
    for( typename BlobsListType::const_iterator i = blobs1.begin(); i != blobs1.end(); ++i )
      ...
      for( typename BlobsListType::const_iterator j = blobs2.begin(); j != blobs2.end(); ++j )
        {
	if ( correspondencematrix( count1, count2 ) == maxcorr && fabs(  (*i)->GetObjectRadius() - (*j)->GetObjectRadius() ) < 1 )
	  {
	  bestblob = ( *j );
	  bestct = count2;
	  }
	count2++;
      ...
      if (  maxcorr > corrthresh && ( bestblob )   && ( image->GetPixel( ( *i )->GetCenter() )   > 1.e-4  )  && ( image2->GetPixel( bestblob->GetCenter() )  > 1.e-4  ) && ( meancorr < 0.175  ) )
	{
	correspondencematrix.set_column( bestct, correspondencematrix.get_column( 0 ).fill( 0 ) );
	antscout << " best correlation " << maxcorr << " rad1 " << ( *i )->GetObjectRadius() << " rad2 " << bestblob->GetObjectRadius( ) << " : " << (RealType) count1 / (RealType) blobs1.size() * 100.0 << "% " << matchpt << " voxval1 " <<  ( *i )->GetCenter()  << " voxval2 " <<  bestblob->GetCenter()  << " meancorr " << meancorr / count2 << std::endl;
...
private:
  ...
  if( argc < 5 )
    ...
    antscout << "  BlobDetector Image1.ext NumberOfBlobs  Optional-Image2-(not-implemented-yet)  :  blob detection by searching for local extrema of the Laplacian of the Gassian (LoG) " << std::endl;
    ...
    antscout << "  TensorAxialDiffusion     : Largest eigenvalue, equivalent to TensorEigenvalue DTImage.ext 2" << std::endl;
    antscout << "    Usage        : TensorAxialDiffusion DTImage.ext" << std::endl;
    antscout << "  TensorEigenvalue         : Gets a single eigenvalue 0-2, where 0 = smallest, 2 = largest" << std::endl;
