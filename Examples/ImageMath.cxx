...
int BlobDetector( int argc, char *argv[] )
{
  ...
  if ( !blobs1.empty() )
    for( typename BlobsListType::const_iterator i = blobs1.begin(); i != blobs1.end(); ++i)
      {
	antscout <<  "Value: " << (*i)->GetScaleSpaceValue() << " sigma of Laplacian detector " << (*i)->GetScaleSpaceSigma() << " sigma of detected Gaussian " << (*i)->GetSigma() << " center "  <<  (*i)->GetCenter() << std::endl;
      }
  antscout << std::endl;
  if ( fn2.length() > 3 ) 
    {
