...
int ConvertTransformFile(int argc, char* argv[])
{
  ...
  // Outputs
  //
  typedef itk::MatrixOffsetTransformBase< typename TransformType::ScalarType, ImageDimension, ImageDimension > OffsetTransformType;
  typename OffsetTransformType::Pointer matrixOffsetTransform = dynamic_cast<OffsetTransformType*> (transform.GetPointer());

  if( outputMatrix || outputHomogeneousMatrix )
    {
    if( matrixOffsetTransform.IsNull() )
      {
      antscout << "The transfrom read from file is not derived from MatrixOffsetTransformBase. Cannot output matrix." << std::endl;
      return EXIT_FAILURE;
      }
