...
bool GetMatrix( const typename TTransform::Pointer & transform, typename TTransform::MatrixType & matrix, bool outputRAS )
{
...
int ConvertTransformFile(int argc, char* argv[])
{
  ...
  if( argc > 4 )
    ...
    if( (outputMatrix && outputHomogeneousMatrix) || (outputMatrix && outputAffine) || (outputHomogeneousMatrix && outputAffine) )
      {
  ...
  if( outputMatrix || outputHomogeneousMatrix )
    ...
    if( outputMatrix )
      ...
      if( GetMatrix<TransformType>( transform, matrix, outputRAS ) )
        ...
        {
        antscout << "Error. Transform type is unsupported for getting matrix: " << transform->GetNameOfClass() << std::endl;
        return EXIT_FAILURE;
        }
      }
    else
  ...
  if( outputAffine )
    ...
    typedef itk::MatrixOffsetTransformBase< typename TransformType::ScalarType, ImageDimension, ImageDimension > CastTransformType;
    typename CastTransformType::Pointer matrixOffsetTransform = dynamic_cast<CastTransformType*> (transform.GetPointer());
    if( matrixOffsetTransform.IsNull() )
      {
      antscout << "The transfrom read from file is not derived from MatrixOffsetTransformBase. Cannot convert to Affine." << std::endl;
      return EXIT_FAILURE;
      }
  ...
  if( itksys::SystemTools::GetFilenameLastExtension(outFilename) != ".txt" &&
      ...
      {
      antscout << "Output filename '" << outFilename << "' must end in '.txt' or '.tfm' for text-format output." << std::endl;
      return EXIT_FAILURE;
      }
  int result = itk::ants::WriteTransform<ImageDimension>( transform, outFilename );
  if( result == EXIT_FAILURE )
