  ...
  for( unsigned int currentStageNumber = 0; currentStageNumber < this->m_NumberOfStages; currentStageNumber++ )
    ...
    if( this->m_ApplyLinearTransformsToMovingImageHeader )
      ...
      this->ApplyCompositeLinearTransformToImageHeader( this->m_CompositeLinearTransformForMovingImageHeader,
                                                        dynamic_cast<ImageBaseType *>( const_cast<ImageType *>( preprocessMovingImage.GetPointer() ) ), false );

      if( this->m_MovingImageMask.IsNotNull() )
        {
        this->ApplyCompositeLinearTransformToImageHeader( this->m_CompositeLinearTransformForMovingImageHeader,
                                                          dynamic_cast<ImageBaseType *>( const_cast<MaskImageType *>( this->m_MovingImageMask->GetImage() ) ), false );
        }
      }
