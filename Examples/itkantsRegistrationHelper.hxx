  ...
  for( unsigned int currentStageNumber = 0; currentStageNumber < this->m_NumberOfStages; currentStageNumber++ )
    ...
    if( this->m_ApplyLinearTransformsToMovingImageHeader )
      ...
      this->ApplyCompositeLinearTransformToMovingImageHeader( this->m_CompositeLinearTransformForMovingImageHeader,
                                                              dynamic_cast<ImageBaseType *>( const_cast<ImageType *>( preprocessMovingImage.GetPointer() ) ) );

      if( this->m_MovingImageMask.IsNotNull() )
        {
        this->ApplyCompositeLinearTransformToMovingImageHeader( this->m_CompositeLinearTransformForMovingImageHeader,
                                                                dynamic_cast<ImageBaseType *>( const_cast<MaskImageType *>( this->m_MovingImageMask->GetImage() ) ) );
        }
      }
...
void
RegistrationHelper<VImageDimension>
::ApplyCompositeLinearTransformToMovingImageHeader( const CompositeTransformType * compositeTransform, ImageBaseType * const image )
{
  if( !compositeTransform->IsLinear() )
    {
...
//   imageTransform->Compose( totalTransform.GetPointer(), false );
  ...
  inverseTotalTransform->SetMatrix( dynamic_cast<MatrixOffsetTransformBaseType *>( totalTransform->GetInverseTransform().GetPointer() )->GetMatrix() );
