...
public:
  ...
  RealType IHTRegression( MatrixType& A, VectorType& x_k, VectorType  b_in, RealType convcrit,
			  unsigned int , RealType mu , bool isp = true , bool verbose = false );
  ...
  { // computes  outer_product( p , p ) * v          
    if ( p.size() != v.size() ) { ::ants::antscout << "FastOuterProductVectorMultiplication Usage Error " << std::endl; return v; }
