		 ...
		 unsigned int maxits, TRealType mu,  bool isp , bool verbose )
{
  ...
  while( ( ( deltaminerr > 0.1 ) && ( approxerr > convcrit ) && ( ct < maxits ) )
    ...
    deltaminerr = ( lasterr - approxerr );
    if ( verbose ) ::ants::antscout << " IHTRegression " << approxerr <<  " deltaminerr " << deltaminerr <<  " ct " << ct << " dataterm " << dataterm << std::endl;
    lasterr = approxerr;
    x_k = x_k1;
    ct++;
    }
...
::NetworkDecomposition(unsigned int n_vecs )
{
  ...
  RealType predictionerror =
    (  predicted
       - this->m_OriginalMatrixR.get_column( 0 ) ).one_norm() / this->m_OriginalMatrixR.rows();
  ::ants::antscout << " overall-mean-abs-prediction-error: " << predictionerror << " correlation " <<  this->PearsonCorr(  predicted , this->m_OriginalMatrixR.get_column( 0 ) )  << std::endl;
...
::SparsePartialArnoldiCCA(unsigned int n_vecs_in)
{
  ...
  while( ( ( loop < maxloop ) ) ) // && ( energyincreases ) ) || loop < 1 ) // 
	...
	qveck = this->Orthogonalize( qveck, qj );
      ...
      ::ants::antscout  << " p " << ptemp.two_norm( ) << " upp " << pveck.two_norm() * this->m_GradStep << " q " << qtemp.two_norm( ) << " upq " << qveck.two_norm() * this->m_GradStep << std::endl;
