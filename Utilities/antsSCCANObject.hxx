...
::NormalizeMatrix( typename antsSCCANObject<TInputImage, TRealType>::MatrixType p  , bool makepositive )
{
  ...
  if ( makepositive ) np = np - np.min_value();
  return np;
}
                       ...
                       typename antsSCCANObject<TInputImage, TRealType>::MatrixType myvariate )
{
  ...
  return ( onenorm - reconerr ) / onenorm;
	...
	::SparseReconPrior(unsigned int n_vecs,bool prior)
	{
		...
		for( unsigned int overit = 0; overit < this->m_MaximumNumberOfIterations; overit++ )
		{
			...
			for(  unsigned int a = 0; a < n_vecs; a++ )
			{
				...
				if ( overit == 0 ) ( void ) this->PowerIteration(  partialmatrix,  evec, 3, true );
				
				if (prior)	  
					this->m_CanonicalCorrelations[ a ] = this->IHTPowerIterationPrior(  partialmatrix,  evec,priorVec, 20  , a,lambda );
				else
					this->m_CanonicalCorrelations[ a ] = this->IHTPowerIterationHome(  partialmatrix,  evec, 20  , a );
							 ...
							 unsigned int maxits , unsigned int maxorth, double lambda )
	{
		...
		VectorType   proj = ( A *((1-lambda)*evec+ lambda*prior)  );
		...
		while( ( ( rayquo > rayquold ) && ( powerits < maxits ) )  )
		{
			...
			evec = this->SpatiallySmoothVector( evec, this->m_MaskImageP, 1. );			//    this->CurvatureSparseness( evec ,  ( 1 - this->m_FractionNonZeroP ) * 100, 5 );
			this->SparsifyP( evec  );
			...
			proj = ( A *((1-lambda)*evec+ lambda*prior) );
			rayquold = rayquo;
...
::SparseRecon(unsigned int n_vecs)
{
  ...
  this->m_CanonicalCorrelations.set_size( n_vecs );
  this->m_CanonicalCorrelations.fill( 0 );
  ::ants::antscout << " sparse recon " << this->m_MinClusterSizeP << " UseL1 " << this->m_UseL1 <<  " Keep+ " <<  this->m_KeepPositiveP << std::endl;
  MatrixType matrixB( this->m_OriginalMatrixP.rows(), n_vecs );
  matrixB.fill( 0 );
  this->m_MatrixP = this->NormalizeMatrix( this->m_OriginalMatrixP );
  this->m_VariatesP.set_size( this->m_MatrixP.cols(), n_vecs );
  ...
  for( unsigned int i = 0; i < n_vecs; i++ )
    {
    VectorType initvec = this->InitializeV( this->m_MatrixP, i + 1 );
    for( unsigned int j = 0; j < i; j++ ) initvec = this->Orthogonalize( initvec, this->m_VariatesP.get_column( j ) );
  ...
  for( unsigned int i = 0; i < n_vecs; i++ )
    ...
    if ( v.min_value() < 0 ) this->m_VariatesP.set_column( i , v * ( -1 ) );
    }
...
{
  if ( this->m_GoldenSectionCounter == 0 )  this->m_GSBestSol = a;
  ...
  */
  ...
  while ( ( relfac > 1.e-4  )  && ( powerits < maxits ) )
    ...
    for( unsigned int orth = 0; orth < maxorth; orth++ )
      ...
      nvec = nvec - this->m_VariatesP.get_column( orth ) * ip1 / ip2;
      */
      }
    nvec = nvec * 0.5 + orthvec * 0.5;
    RealType gamma = 0.1;
    RealType smooth = 0.5;
    if ( smooth > 0 ) nvec = this->SpatiallySmoothVector( nvec, this->m_MaskImageP, smooth );
    if( ( lastgrad.two_norm() > 0  ) && ( conjgrad ) )
      {
      gamma = inner_product( nvec, nvec ) / inner_product( lastgrad, lastgrad );
      }
  ...
  evecin = bestevec;
  ::ants::antscout << "rayleigh-quotient: " << bestrayquo << " in " << powerits << " num " << maxorth << std::endl;
  return bestrayquo;
}
                                 ...
                                 unsigned int maxits, bool makesparse )
{
  ...
  /** Here, should be more careful to make sure energy improves, need better command line interface for specifying objective */ 
		 ...
		 unsigned int maxits, TRealType mu,  bool isp , bool verbose )
{
  ...
  RealType     starterr = 1.e99;
  RealType     minerr = starterr, deltaminerr = 1, lasterr = starterr * 2;
  if ( isp ) this->SparsifyP( x_k ); else this->SparsifyQ( x_k );
  RealType alpha_k = 1.e-4 / A.frobenius_norm();
  if ( verbose ) ::ants::antscout << " alpha_k " << alpha_k << " xknorm " << x_k.two_norm() << " Anorm " << A.frobenius_norm() << std::endl;
  RealType gamma = 0;
  bool conjgrad = true;
  ...
  while(  ( ct < maxits ) && alpha_k > 1.e-14 )
    ...
    deltaminerr = ( lasterr - approxerr );
    if ( verbose ) ::ants::antscout << " IHTRegression " << approxerr <<  " deltaminerr " << deltaminerr <<  " ct " << ct << " dataterm " << dataterm <<  " alpha_k " << alpha_k << std::endl;
    lasterr = approxerr;
    x_k = x_k1;
    ct++;
    }
...
::NetworkDecomposition(unsigned int n_vecs )
{
  ...
  for( unsigned int fold = 0; fold < foldnum;  fold++ )
	...
	minerr1 = this->IHTRegression(  matrixP,  randv, bp, 0, 500 , 0 , true, true ); /** good */       
      ...
      if( predct > 0 )
        ...
        ::ants::antscout << "Fold: " << fold << " minerr," << loerror << ",col," << colind
                         << " totalpredictionerr " << avgprederr / predct <<  " ,fiterr, " << fiterr << " Gap: " <<  ( avgprederr / predct ) - fiterr  << std::endl;
        }
      else
        {
    ...
    RealType wt = ( ( soln - r_leave_out.get_column( 0 ) ).one_norm() / soln.size() ) / (  ( r_leave_out.get_column( 0 ) ).one_norm() / soln.size() );
  ...
  RealType predictionerror =
    (  predicted
       - this->m_OriginalMatrixR.get_column( 0 ) ).one_norm() / this->m_OriginalMatrixR.rows();
  ::ants::antscout << " overall-mean-abs-prediction-error: " << predictionerror << " correlation " <<  this->PearsonCorr(  predicted , this->m_OriginalMatrixR.get_column( 0 ) )  << " Gap " <<  avgprederr / predct - avgfiterr  << std::endl;
...
::SparsePartialArnoldiCCA(unsigned int n_vecs_in)
{
  ...
  this->m_CanonicalCorrelations.set_size(n_vecs);
  this->m_CanonicalCorrelations.fill(0);
  ::ants::antscout << " arnoldi sparse partial cca : L1?" << this->m_UseL1 << " GradStep " << this->m_GradStep << std::endl;
